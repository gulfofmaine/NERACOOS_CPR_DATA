...
This folder contains documentation on the Gulf of Maine CPR data obtained directly from the Marine Biological Association (MBA), and adjustments made prior to its integration into ERDDAP. These include a reshaping to a tidy-data format, and the renaming of a handful of columns to more generalized titles.
...
Data processing and reshaping steps have been included in `_targets.R` with the supporting functions written and notated in `targets_R/targets_support.R`

## ERDDAP Metadata

The `.xml` files associated with the MBA datasets can be found in the `erddap_xml folder`. They detail the units and column specifications that ERDDAP uses when serving the data and handling queries.
