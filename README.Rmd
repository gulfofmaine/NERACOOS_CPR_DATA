...
Raw data from all sources is contained in the `data_raw/` directory. Code that prepares the raw data for ERDDAP and any necessary documentation is specific to the source that the data was recieved from. This information can be found in the following sub-folders:
...
#tar_visnetwork()
withr::with_dir(rprojroot::find_root('_targets.R'), tar_glimpse())
```
...
Conversions to a standard unit of measurement is necessary when working together with CPR obtained from these two sources. Another additional hurdle is that the taxa and group stages are also inconsistently used across them, and require a key for transitioning to coarser scale groups.
