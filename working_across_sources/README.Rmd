...
withr::with_dir(rprojroot::find_root('_targets.R'), tar_load("noaa_zooplankton_erddap"))
withr::with_dir(rprojroot::find_root('_targets.R'), tar_load("mba_eyecount_erddap"))
withr::with_dir(rprojroot::find_root('_targets.R'), tar_load("mba_traverse_erddap"))
```
...
## 1. Totalling MBA Zooplankton

Data from the MBA preserves the two measurement scales that the zooplankton data is recorded in. To get total zooplankton abundances.
...
mba_zoo <- full_join(
  x = mba_trav, 
  y = mba_eye, 
  # by = c("cruise", "transect_number", "date", "latitude", "longitude", "taxon", "taxa_id_number") # Target name
  by = c("cruise", "transect_number", "time", "latitude", "longitude", "taxon", "taxa_id_number") # Erddap names
)
...
mba_zooplankton <- mba_zoo %>% 
  mutate(
    ...
    abundance_per_transect = trans_abund_traverse + trans_abund_eyecount,
    abundance              = abund_traverse + abund_eyecount
  ) %>% 
  select(-c(abund_eyecount, abund_traverse, trans_abund_eyecount, trans_abund_traverse))
...
## 2. Matching Development Stage Differences
...
# # This way doesn't work because the taxa from MBA are longer than the taxa were trying to  match agaisnt
# mba_test %>%
#   split(.$taxa) %>% 
